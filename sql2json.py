...
if yaml_config["geo_api"]["geolocator"] == "GoogleV3":
    geolocator = GoogleV3(api_key=yaml_config["geo_api"]["api_key"])
else:
    geolocator = Nominatim(
...
def find_group_name(find_groups_name):
    dict_group_name = {}
    count = 1
...
for data in get_main_data():
    panel_id = data[0]
    create_obj_date = data[1]
    area_name = data[2]
    master_name = data[3]
    company_name = data[4]
    object_address = data[5]
    contract_id = data[6]
    remarks = data[8]
    phone_number = data[9]
...
try:
    with open("json_output\\converted_sql_{0}.json".format(str(datetime.now().strftime("%d-%m-%Y %H-%M-%S"))), "w", encoding="utf8") as outfile:
